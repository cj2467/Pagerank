...
class WebGraph():
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=10, epsilon=1e-6):
        '''
 ...
 #           print(v.shape)
  #          print(self.P.shape)
            ...
            while (num_iterations<max_iterations):
                ...
                # equation 5.1
                xk = (alpha*torch.sparse.mm(self.P.t(),x).t() + (alpha*torch.sparse.mm(self.P.t(),x).t() + (1-alpha))*v.t()).t()

                # check error
                if torch.norm(xk-x) < epsilon:
                    break
